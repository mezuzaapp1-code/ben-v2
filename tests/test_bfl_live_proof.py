...
pytestmark = pytest.mark.skipif(
    os.getenv("BEN_SINGLE_BFL_PROOF") != "authorized-one-image",
    reason="single paid image requires explicit protected workflow")


@pytest.mark.asyncio
async def test_single_authorized_bfl_image(repository, monkeypatch, tmp_path):
    if os.getenv("BEN_SINGLE_BFL_PROOF") != "authorized-one-image":
    ...
    assert os.getenv("BFL_API_KEY"), "Protected credential missing"
    ...
    from services.media.contracts import BFL_IMAGE_MODEL
    ...
    bfl = service.bfl_adapter
    generate = bfl.submit
    poll = bfl.poll
    metrics = {}
    import services.media.service as service_module
    original_ingest = service_module.ingest_png
    ...
    def measured_ingest(*args, **kwargs):
        ...
        try:
            ...
            metrics["ingestion_duration_ms"] = round((time.monotonic() - started) * 1000, 2)

    async def measured_poll(*args, **kwargs):
        result = await poll(*args, **kwargs)
        if result[0] == "Ready":
            metrics["submission_to_ready_ms"] = round((time.monotonic() - metrics["submission_started"]) * 1000, 2)
        return result

    monkeypatch.setattr(service_module, "ingest_png", measured_ingest)
    monkeypatch.setattr(bfl, "poll", measured_poll)
    calls = 0

    async def one_submission(request):
        nonlocal calls
        calls += 1
        assert calls == 1 and request.model == BFL_IMAGE_MODEL
        metrics["submission_started"] = time.monotonic()
        submitted = await generate(request)
        metrics["submission_accepted"] = True
        return submitted

    monkeypatch.setattr(bfl, "submit", one_submission)
    app = FastAPI()
    ...
    evidence = {"status": "FAIL", "commit": os.getenv("GITHUB_SHA"),
                "provider": "bfl", "requested_model": BFL_IMAGE_MODEL,
    ...
    try:
        async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url="http://ben-proof") as client:
            ...
            # Recreate the service after submission; reconcile only persisted operation state.
            service = MediaService(repo, bfl_adapter=bfl)
            observed_states = []
            while True:
                current = await repo.read(org, alias, execution=uuid.UUID(execution))
                evidence.update(state=current["state"], error_code=current["error_code"],
                    execution_id=execution, poll_attempts=current["poll_attempts"],
                    usage=current["usage_dimensions"])
                observed_states.append(current["state"])
                if current["state"] in ("succeeded", "failed", "expired", "submission_unknown"):
                    break
                assert current["error_code"] is None, "Stop proof on first reconciliation error"
                assert time.monotonic() - start < 360, "Proof observation deadline reached; never resubmit"
                delay = max(0.1, (current["next_reconcile_at"] - datetime.now(timezone.utc)).total_seconds())
            ...
            assert row["provider"] == "bfl" and row["model"] == BFL_IMAGE_MODEL
            ...
            evidence.update(status="PASS", resource_id=resource, byte_size=row["byte_size"],
                checksum=row["checksum"], dispatch_model=observation["upstream_model"],
                model_identity_source=observation["model_identity_source"],
                ...
                checks=["authorization", "tenant_rls", "idempotency", "one_submission", "ingestion",
                        "protected_delivery", "conversation_history", "persisted_restart_read", "durable_polling_restart", "accounting", "provenance", "evaluation"])
    except Exception as exc:
        evidence["failure_type"] = type(exc).__name__
        raise
    finally:
        evidence.update(phase=phase, provider_submissions=calls,
            total_proof_duration_ms=round((time.monotonic() - proof_started) * 1000, 2),
            submission_accepted=metrics.get("submission_accepted", False),
            submission_to_ready_ms=metrics.get("submission_to_ready_ms"),
            ingestion_duration_ms=metrics.get("ingestion_duration_ms"))
