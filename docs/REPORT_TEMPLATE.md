...
- **PASS** Γאפ Executed; criteria met.
- **FAIL** Γאפ Executed; criteria not met (include exact error).
- **PARTIAL** Γאפ Some criteria met; list what passed and what failed.
- **NOT VERIFIED** Γאפ Not executed or blocked (state why).
...
1. **Never claim verification without execution** Γאפ If a command was not run, mark **NOT VERIFIED**.
2. **Never hide failures** Γאפ Report FAIL and PARTIAL explicitly with exact errors.
3. **Never print secrets** Γאפ No API keys, tokens, or full `DATABASE_URL` with credentials.
4. **Separate VERIFIED vs INFERRED** Γאפ Production behavior inferred from smoke is VERIFIED only after the smoke runs; Railway dashboard settings are INFERRED until checked.
5. **Include exact errors** Γאפ HTTP status, stderr, exception type, and message (redact secrets).
6. **List operational risks explicitly** Γאפ Migration rollback, API cost, degraded expert paths, missing health endpoint, etc.
