...
clearPendingMedia(storage, 'bfl-conversation')
const bfl = pendingMedia(storage, 'bfl-conversation', { ...intent, model: 'flux-2-pro' })
await mediaRequest('/executions', {}, { body: bfl })
assert.equal(JSON.parse(captured.options.body).model, 'flux-2-pro')
assert.equal(pendingMedia(storage, 'bfl-conversation', intent).model, 'flux-2-pro')
await mediaRequest('/resources/ben-owned/content', {}, { binary: true })
assert.equal(captured.options.cache, 'no-store')
const video = pendingMedia(storage, 'veo-conversation', { ...intent, model: 'veo-3.1-fast-generate-preview',
  source_resource_id: 'ben-source-image', duration_seconds: 4, resolution: '720p', aspect_ratio: '16:9' })
await mediaRequest('/executions', {}, { body: video })
