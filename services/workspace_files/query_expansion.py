...
def expansion_trace(tokens: Sequence[str], *, mode: str | None = None) -> dict:
    """Observability for query-prep expansion. Tokens only, never file text."""
    resolved = (mode or lexical_expand_mode()).strip().lower()
    safe_tokens = [str(t).strip().lower() for t in tokens if str(t).strip()]
    extras = extra_tsquery_atoms(safe_tokens, mode=resolved)
    return {
        "lexical_expand_mode": resolved if resolved in _ALLOWED_MODES else "off",
        "query_tokens": tuple(safe_tokens[:12]),
        "expansion_atom_count": len(extras),
    }
